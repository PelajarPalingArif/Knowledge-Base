...
docker run -d --name %container_name% -p %port%:3306 -e MYSQL_ROOT_PASSWORD=%root_password% -e MYSQL_DATABASE=%database% -v %data_dir_path%:/var/lib/mysql -v %config_file_path%:/etc/mysql/my.cnf mysql:%version%
